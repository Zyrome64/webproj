...
def find(coords, type_):
    response = None
    try:
        map_request = 'https://search-maps.yandex.ru/v1/?text=Станция метро&ll={},{}&spn=1.0,1.0&results=1&type={}&lang=ru_RU&apikey=dda3ddba-c9ea-4ead-9010-f43fbc15c6e3'.format(coords[0], coords[1], type_)
        response = requests.get(map_request)
        if response:
            json_response = response.json()
            name = json_response['features'][0]['properties']['CompanyMetaData']['name']
            adress = json_response['features'][0]['properties']['CompanyMetaData']['address']
            bounds = json_response['features'][0]['properties']['boundedBy']
            print('Название - ', name, '\n', 'Адрес - ', adress, '\n', 'Координаты - ', str((bounds[0][0] + bounds[1][0]) / 2), ', ', str((bounds[0][1] + bounds[1][1]) / 2), sep='')
        if not response:
            print("Http статус:", response.status_code, "(", response.reason, ")")
            sys.exit(1)
    except:
